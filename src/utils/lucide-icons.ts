...
const iconSets: Record<string, ThemeIconSet> = {
  default: {
    ...
    FilePlusCorner:
      '<path d="M11.35 22H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v5.35"/><path d="M14 2v5a1 1 0 0 0 1 1h5"/><path d="M14 19h6"/><path d="M17 16v6"/>',
    FolderOpenDot:
  ...
  fresh: {
    ...
    ArrowUpDown:
      '<line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/><polyline points="5 12 12 5 19 12"/>',
    ChevronsUpDown:
    ...
    ArrowDownAZ:
      '<polyline points="18 9 22 13 18 17"/><polyline points="14 9 14 17"/><path d="M10 9H6l4 8H6"/>',
    ChevronRight:
...
export function setIconTheme(theme: string): void {
  if (iconSets[theme]) currentTheme = theme;
}
...
export function createLucideIcon(name: string, size: number = 16): string {
  ...
  if (!iconPath) return "";
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none">${iconPath}</svg>`;
}
